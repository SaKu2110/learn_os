...
// Avoiding the implicit declaration of kill()
...
static int check_args(int nproc, int total, int resol)
{
    int flag = 0;
    if (nproc < 1)
    {
...
int main(int argc, char *argv[])
{
    ...
    if(check_args(nproc, total, resol) != 0)
        exit(EXIT_FAILURE);
    ...
    if (!logbuf)
        err(EXIT_FAILURE, "malloc(logbuf) faild\n");

    puts("estimating workload witch tass just one milisecond");
